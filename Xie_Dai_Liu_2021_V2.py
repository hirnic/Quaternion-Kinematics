...
def PoseFinderGN(init, lengths, TableID, Base):
    ...
    while not found and k < kmax:
        k += 1
        print("T = ", T)
        slm = np.linalg.solve(A + mu * np.eye(6), -g)
        S_lm = screw_matrix(slm)
        if np.linalg.norm(slm) <= eps2:
            found = True
        else:
            T_candidate= expm(S_lm) @ T
            r_candidate = residual_vector(T_candidate, lengths, TableID, Base)
            F_old = np.dot(r, r)
            F_new = np.dot(r_candidate, r_candidate)
            delta_F = F_old - F_new
            delta_L = r - r_candidate
            if delta_F / delta_L > 0:
                T = T_candidate
                J = Jacobian(T, TableID, Base)
                A = J.T @ J
                r = residual_vector(T, lengths, TableID, Base)
                g = J.T @ r
                found = (np.max(np.abs(g)) <= eps1)
                mu = mu * max(1 / 3, 1 - (2 * delta_F / F_old - 1) ** 3)
                nu = 2
...
# Due to poorly written pseudocode and no mention of the actual algorithm in the paper, this script has an untenable
# bug that I do not have time to delve into learning how to fix.
